...
struct Client {
  ...
  int bw, oldbw;
  unsigned int tags;
  int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isfullscreengeometry;
...
static void setfullscreengeomtry(Client *c, int fullscreen);
...
static void togglefullscrgeometry(const Arg *arg);
...
void movemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  if (c->isfullscreengeometry) /* no support moving fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
...
void resizemouse(const Arg *arg) {
  ...
  if (!(c = selmon->sel))
    return;
  if (c->isfullscreengeometry) /* no support resizing fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
...
void setfullscreengeomtry(Client *c, int fullscreen) {

  if (fullscreen && !c->isfullscreengeometry) {
    c->isfullscreengeometry = 1;
    ...
    XRaiseWindow(dpy, c->win);
  } else if (!fullscreen && c->isfullscreengeometry) {
    c->isfullscreengeometry = 0;
...
void setlayout(const Arg *arg) {
  ...
  memcpy(selmon->ltsymbol, selmon->lt[selmon->sellt]->symbol,
...
void togglefloating(const Arg *arg) {
  if (!selmon->sel)
    return;
  if (selmon->sel->isfullscreengeometry) /* no support for fullscreen windows */
    return;
...
void updatesystray(void) {
  ...
  for (w = 0, i = systray->icons; i; i = i->next) {
    ...
    if (textsystray) {
      XClassHint ch = {NULL, NULL};
      if (!XGetClassHint(dpy, i->win, &ch) || !ch.res_name)
        label = strdup("?");
      else {
        label = calloc(sizeof(char), 3);
			strncpy(label, ch.res_class, 2);
		 /* label[0] = tolower(label[0]); */
		}
      drw_text(drw, 0, 0, i->w, bh, lrpad / 4 , label, 0);
